...
if(isset($_GET['id'])) {
    ...
    if($agent) {
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $edu = $agent['edu'];
            if ($_FILES['edu']['error'] === UPLOAD_ERR_OK) {
                $edu_tmp = $_FILES['edu']['tmp_name'];
                $edu_name = basename($_FILES['edu']['name']);
                $edu_destination = 'uploads/' . $edu_name;
                if (move_uploaded_file($edu_tmp, $edu_destination)) {
                    $edu = $edu_destination;
                } else {
                    echo "Error uploading Bank file.";
                }
            ...
            $update_query = "UPDATE agents SET agentCode=?, name=?, phone=?, alt_phone=?, email=?, dob=?, doj=?, address=?, companyName=?, aadhar=?, pan=?, photo=?, bank=?,edu=? WHERE id=?";
            $stmt = $connect->prepare($update_query);
            if ($stmt === false) {
                die('Error preparing update statement: ' . htmlspecialchars($connect->error));
            }
            $stmt->bind_param("sssssssssssssss", $agentCode, $name, $phone, $alt_phone, $email, $dob, $doj, $address, $companyName, $aadhar, $pan, $photo, $bank,$edu, $agent_id);
...
<div class="page-wrapper">
    ...
    while($row = $result->fetch_assoc()) {
        ...
        // Check if this company matches the current companyName
        $selected = ($company == htmlspecialchars($agent['companyName'])) ? 'selected' : '';
